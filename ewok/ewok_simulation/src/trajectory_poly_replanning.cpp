...
ewok::PolynomialTrajectory3D<10>::Ptr local_traj;
ewok::EuclideanDistanceRingBuffer<POW>::Ptr edrb;
ewok::Polynomial3DOptimization<10>::Ptr to;

ros::Publisher occ_marker_pub, free_marker_pub, dist_marker_pub, current_traj_pub,trajectory_point_pub;
tf::TransformListener * listener;


rotors_control::PolyLeePositionControllerNode* lee_position_controller_node_ptr;
...
void sendCommandCallback(const ros::TimerEvent& e) {
    ...
    to->getClosestTrajectoryTime();
    to->getMarker(traj_marker);
    current_traj_pub.publish(traj_marker);


    typename ewok::Polynomial3DOptimization<10>::Vector3Array local_path;
...
int main(int argc, char** argv){
    ros::init(argc, argv, "trajectory__poly_replanning");
    ros::NodeHandle nh;
    ros::NodeHandle pnh("~");
    ...
    current_traj_pub = nh.advertise<visualization_msgs::Marker>("optimal_trajectory", 1, true);

    lee_position_controller_node_ptr= new rotors_control::PolyLeePositionControllerNode(nh);
    ...
    ROS_INFO("dt: %f, num_opt_points: %d", dt, num_opt_points);
     ...
     to.reset(new ewok::Polynomial3DOptimization<10>(limits*0.6, nh));
...

    {
        ...
        lee_position_controller_node_ptr->SetTraj(traj);
