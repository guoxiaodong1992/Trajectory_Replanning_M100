 ...
 public:
  ...
  _Scalar optimize() {
    ...
    if(trajectory_.get()) {
      ...
      //output test
      std::cout<<std::endl<<"max time"<<spline_.maxValidTime()<<"min time " <<spline_.minValidTime() <<"end_point time: "<<enpoint_time<<"position: "<<endpoints[0]
               <<"velocity: "<<endpoints[1]<<std::endl;
    }
