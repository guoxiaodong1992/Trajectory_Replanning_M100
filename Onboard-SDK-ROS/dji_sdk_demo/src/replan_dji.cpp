...
#include "dji_sdk_demo/replan_dji.h"
...
ros::Publisher executeStatusPub;
...
geometry_msgs::Point cmdPoint;
std_msgs::Bool executeStatus;
double start_x, start_y, start_z, start_yaw;


int main(int argc, char** argv)
{
  ros::init(argc, argv, "dji_trajectory_replanning_quadrotor");
  ros::NodeHandle nh;
  ros::NodeHandle pnh("~");
  ...
  ros::Subscriber cmdPointSub   =nh.subscribe("command/point",10,&cmd_point_callback);
  ...
  cout<<"M100 Trajectory Replanning Node! Wish You Successful!"<<endl;



  // Publish the control signal
  ctrlPosYawPub = nh.advertise<sensor_msgs::Joy>("/dji_sdk/flight_control_setpoint_ENUposition_yaw", 10);
  executeStatusPub=nh.advertise<std_msgs::Bool>("dji_sdk/set_execute_status",10);
  // Basic services
  ...
  if (elapsed_time > ros::Duration(0.02)) {
    ...
    if (target_set_state == 1) {
      ...
      if (current_gps_health > 3) {
        setTarget(start_x, start_y, start_z, start_yaw);
    ...
    if (target_set_state == 2) {
      //! follow the Replanning Trajectory
      if (current_gps_health > 3) {
        setTarget(cmdPoint.x, cmdPoint.y, cmdPoint.z, start_yaw);
        local_position_ctrl(xCmd, yCmd, zCmd);
      }
      else
      {
...
void local_position_ctrl(double &xCmd, double &yCmd, double &zCmd)
{
  ...
  if (((std::abs(xCmd)) < 0.1) && ((std::abs(yCmd)) < 0.1) &&
    ...
    if(target_set_state <= num_targets) {
      ...
      {
        target_set_state=2;
        executeStatus.data=true;
        executeStatusPub.publish(executeStatus);
      }
    }
    else
    {
